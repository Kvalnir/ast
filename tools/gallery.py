...
<div class="gal">
  ...
  <header class="galhero">
    ...
    <p class="lede">One clean example per technique is not enough to train an eye. Here is each of the nine on nine boards: <b>six are the real thing and three are not</b>, shuffled, answer hidden. Read the figure, decide, then tap it.</p>
    <p class="lede">The near misses are the point. None is manufactured: each is a real position holding what gets mistaken for the technique — a box with one stray spot, three cells pooling to four digits, two lines that meet twice — and each is checked to contain no instance of the technique it is filed under, so a <b>no</b> is a no about the whole picture. Tell those three from the six and you can trust yourself at the puzzle.</p>
