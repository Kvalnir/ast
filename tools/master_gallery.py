...
BUG_SECTION = '''
  <section class="galsec" id="bug">
    <div class="galintro">
      ...
      <p class="what">%s</p>
      <p class="guard"><span>Why there is no drill here</span>BUG+1 is not a pattern you can point
        at, and cannot be made into one. It is a statement about <b>every unsolved square at once</b> — all of them
...
<div class="gal">
  ...
  <header class="galhero">
    ...
    <p class="lede">The <a href="gallery.html">Advanced gallery</a>'s drill, one tier up, and harder in a particular way: here the near misses are <b>sound readings of the wrong thing</b> — a rectangle that cannot swap, fins in two boxes, a chain that comes back on the wrong digit. Six of each nine are real. Read the figure, decide, then tap it.</p>
    <p class="lede">Two kinds of figure. A single-digit technique — fins, kites, empty rectangles, colouring — shows one digit's whole map, the way the News+ highlight does. A chain shows only its own squares and their marks: the question is whether the links hold, not whether you can find them.</p>
...
%s
  ...
  <footer>
    <p>Cover the captions, work a row of nine, and only then check. When the pattern you are unsure about is one you are actually looking at rather than one of these, the <a href="check.html">pattern check</a> audits it condition by condition — and on this tier most of what it reports is a count only your own grid can settle. Square references use <code>r5c3</code> for row 5, column 3.</p>
  </footer>
