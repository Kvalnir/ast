...
<div class="sitebar">
  ...
  <p class="lede">The <a href="index.html" style="color:var(--amber)">Advanced nine</a> get you through every News+ challenging puzzle and most of what an app calls expert. Past that, a puzzle will occasionally hold exactly one move none of them can find. These are those moves: nine more, and seven of them are one idea in different geometry.</p>
  <p class="lede">The idea is the <b>strong link</b>: a unit where a digit has exactly two possible squares, so one of them is true. You already use it — a skyscraper is two strong links sharing an end. Learn it properly and the names below stop being nine things to remember.</p>
  <p class="lede" style="margin-bottom:20px">The other two — the <a href="#unique-rectangle" style="color:var(--amber)">unique rectangle</a> and <a href="#bug" style="color:var(--amber)">BUG+1</a> — reason from the puzzle having one answer rather than from the grid in front of you, so they are the only techniques here that can be confidently wrong on a grid you have already broken. <a href="#uniqueness" style="color:var(--amber)">More on that below</a>; the trainer refuses to offer either unless your board still leads to a single solution.</p>
  <div class="legend">
